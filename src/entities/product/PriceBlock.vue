<template>
  <div>
    ...
    <div class="rounded-xl bg-gray-100 p-4">
      ...
      <p class="font-semibold flex items-center" v-if="product.inStock !== 'instock'">
      ...
      <Transition name="list" mode="out-in">
        <!-- in cart -->
        <div class="mt-4 flex items-center justify-between" key="edited" v-if="productInCart(product.id)">
          ...
          <button class="bg-gray-300 rounded-xl w-14 px-4 py-4 whitespace-nowrap font-bold"
            @click="addCartItem(product, -1)" :disabled="inProgress">
            <MinusIcon />
          </button>
        ...
        <button class="bg-primary rounded-xl px-4 py-4 text-white whitespace-nowrap font-bold w-full mt-4"
          @click="addCartItem(product, 1)" :disabled="inProgress" v-else
