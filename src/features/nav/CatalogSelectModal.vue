<template>
  <div
    class="fixed top-0 left-0 h-full w-full bg-[rgba(0,0,0,.5)] z-20 flex justify-center items-center text-black"
    @click="closeModal"
  >
    <div
      class="bg-white rounded-lg p-8 max-w-[400px] relative"
      @click.stop
    >
      <div
      ...
      <p class="text-sm font-semibold text-gray-500 mb-6">
        Выберите адрес, чтобы видеть актуальные цены и наличие
        товаров в
        интернет-магазине
      </p>
      <div class="space-y-4">
        <div
          v-for="(item, index) in catalogTypes"
          :key="index"
          class="rounded p-4 ring-1 ring-gray-300 font-medium transition-all cursor-pointer relative"
          :class="{ 'ring-2 ring-primary_2': catalogSelected === item.value }"
          @click="changeCatalog(item.value)"
        >
          <CheckIcon
            v-if="catalogSelected === item.value"
            class="text-primary_2 h-4  absolute top-2 right-2"
          />
