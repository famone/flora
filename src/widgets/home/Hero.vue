<template>
    <div class="pt-8">
        <div class="container mx-auto relative">
            <button :class="sliderBtn" class="-left-12 prev-slide">
                <ArrowSmallLeftIcon class="h-6" />
            </button>
            <button :class="sliderBtn" class="-right-12 next-slide">
                <ArrowSmallRightIcon class="h-6" />
            </button>
            <swiper :slides-per-view="1" :space-between="24" :navigation="{ nextEl: '.next-slide', prevEl: '.prev-slide', }"
                :modules="modules">
                <swiper-slide>
                    <div class="h-[40rem] rounded-xl overflow-hidden bg-gray-200 slide-box p-24 flex items-center relative">
                        <video-background src="https://sflora.ru/wp-content/uploads/2022/12/flora_movie.mp4"
                            class="video-bg">
                        </video-background>
                        <div class="relative">
                            <p class="text-lg text-white font-semibold tracking-widest mb-6">НОВОСТИ</p>
                            <h1 class="text-4xl font-bold text-white max-w-[70%] mb-8">
                                {{ starterSlide.name }}
                            </h1>
                            <p class="text-white max-w-[60%]" v-html="starterSlide.descr"></p>
                        </div>
                    </div>
                </swiper-slide>
                <swiper-slide v-for="(slide, index) in content.news" :key="index">
                    <div class="h-[40rem] rounded-xl overflow-hidden bg-gray-200 slide-box p-24 flex items-center"
                        :style="`background-image: url('${slide.mainImage}');`">
                        <div class="relative">
                            <p class="text-lg text-white font-semibold tracking-widest mb-6">НОВОСТИ</p>
                            <h1 class="text-4xl font-bold text-white max-w-[70%] mb-8">{{ normalizeString(slide.name) }}
                            </h1>
                            <p class="text-white max-w-[60%]" v-html="slide.descr"></p>
                        </div>
                    </div>
                </swiper-slide>
            </swiper>
