<template>
  <header class="header shadow-md mx-auto sticky top-0 z-20">
    <div v-if="overlay" class="h-screen w-screen fixed top-0 left-0 bg-black opacity-50 -z-10"
    ...
    <div class="bg-white py-2">
      <div class="container mx-auto">
        <div class="flex gap-6 justify-between items-center">
          ...
          <MainSearch @on-focus="overlay = true" @on-focusout="overlay = false" />
          <HeaderActions />
        </div>
      </div>
    </div>
    <div class="bg-white pb-5">
      <div class="container mx-auto flex justify-between items-center">
        ...
        <div class=" gap-2 flex">
          <router-link :to="btn.link" v-for="(btn, i) in buttons" :key="i">
            <button :class="btn.style">
              <component :is="btn.icon" class="h-4 mr-1" />
              {{ btn.txt }}
            </button>
          </router-link>
        </div>
